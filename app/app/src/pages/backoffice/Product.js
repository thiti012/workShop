...
function Product() {
    ...
    return<BackOffice>
        ...
        <button onClick={clearFormExcel} className="btn btn-success" data-toggle='modal' data-target='#modalExcel'>
            <i className="fa fa-arrow-down mr-2"></i>import from Excell
        </button>

        <table className="mt-3 table table-borderred table-striped">
            <thead>
                <tr>
                    <th>ภาพสินค้า</th>
                    <th >name</th>
        ...
        <MyModal id='modalExcel' title='เลือกไฟล์' >
            <div>เลือกไฟล์</div>
            <input className="form-coontrol" type="file" ref={refExcel} onChange={e => selectedFileExcel(e.target.files)}/>
